...
async def training_intel(
    ...
    **kwargs,
    ...
    skill = SkillLevel(skill_level.lower()) if skill_level.lower() in [s.value for s in SkillLevel] else SkillLevel.NORMAL
    ...
    if action == "status":
        ...
        return translator.translate_gpu_status(
            metrics.gpu_utilization, metrics.gpu_memory_used, metrics.gpu_memory_total, skill
        )
    elif action == "issues":
        if analysis.issues:
...
async def training_viz(
    ...
    **kwargs,
    ...
    output_format = OutputFormat(format.lower()) if format.lower() in [f.value for f in OutputFormat] else OutputFormat.ASCII
...
def _get_gpu_comparison(budget: float) -> str:
    ...
    return f"""# GPU Comparison (Budget: /hr)
