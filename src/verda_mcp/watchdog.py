...
class WatchDogReport:
    ...
    def create_report(
        ...
        content = [
            ...
            f"**Training Status**: {'✅ RUNNING' if training_running else '❌ NOT RUNNING'}",
            "",
            "---",
            "",
            "## 🖥️ GPU Status",
        ...
        if extra_info:
            content.append("")
            content.append("## 📋 Additional Info")
            content.append("```json")
...
async def _do_watchdog_check(instance_ip: str) -> dict:
    ...
    try:
        # GPU status
        results["gpu_status"] = await loop.run_in_executor(None, lambda: manager.get_gpu_status(instance_ip))
    ...
    try:
        # Disk space
        stdout, _, _ = await loop.run_in_executor(None, lambda: manager.run_command(instance_ip, "df -h /workspace"))
    ...
    try:
        # Memory usage
        stdout, _, _ = await loop.run_in_executor(None, lambda: manager.run_command(instance_ip, "free -h"))
    ...
    while _watchdog_enabled:
        try:
            ...
            if not results["training_running"]:
                logger.warning("⚠️ WatchDog Alert: Training is NOT running!")
...
async def get_watchdog_status() -> str:
    ...
    if _watchdog_enabled:
        return f"""# 🐕 WatchDog Status: ENABLED
...
**Latest Report**: {latest.name if latest else "None yet"}"""
        ...
        return f"""# 🐕 WatchDog Status: DISABLED
...
async def manual_watchdog_check(instance_ip: str) -> str:
    ...
    status = "✅ RUNNING" if results["training_running"] else "❌ NOT RUNNING"

    return f"""# 🐕 Manual WatchDog Check Complete
