...
DATASETS = {
    ...
    "dolly-15k": DatasetInfo("databricks/databricks-dolly-15k", DatasetSource.HUGGINGFACE, 0.01, 15000, "json", "Instruction tuning"),
    "openassistant": DatasetInfo("OpenAssistant/oasst1", DatasetSource.HUGGINGFACE, 0.1, 161000, "json", "Conversations"),
    "squad": DatasetInfo("rajpurkar/squad", DatasetSource.HUGGINGFACE, 0.03, 100000, "json", "QA"),
    "code-alpaca": DatasetInfo("sahil2801/CodeAlpaca-20k", DatasetSource.HUGGINGFACE, 0.02, 20000, "json", "Code instructions"),
    "ultrachat": DatasetInfo("stingning/ultrachat", DatasetSource.HUGGINGFACE, 2.0, 1500000, "json", "Multi-turn chat"),
    "slim-orca": DatasetInfo("Open-Orca/SlimOrca", DatasetSource.HUGGINGFACE, 0.8, 518000, "json", "Diverse instructions"),
    "magicoder": DatasetInfo("ise-uiuc/Magicoder-OSS-Instruct-75K", DatasetSource.HUGGINGFACE, 0.1, 75000, "json", "Code generation"),
    "wikitext": DatasetInfo("wikitext", DatasetSource.HUGGINGFACE, 0.5, 4500000, "text", "Language modeling"),
    "fineweb": DatasetInfo("HuggingFaceFW/fineweb", DatasetSource.HUGGINGFACE, 44000, 15000000000, "parquet", "Web pretraining"),
}
...
def dataset_tools(action: str = "list", dataset: str = "", **kwargs) -> str:
    ...
    if action == "list":
        ...
        results = [f"{k}: {d.description}" for k, d in DATASETS.items() if dataset.lower() in k or dataset.lower() in d.description.lower()]
