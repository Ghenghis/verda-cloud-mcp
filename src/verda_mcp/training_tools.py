...
class SpotCheckpointCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if (time.time() - self.last_checkpoint_time) >= self.checkpoint_interval:
            ...
            self.last_checkpoint_time = time.time()
            print(f"⏰ Time-based checkpoint triggered at step {{state.global_step}}")
        return control
...
class TimeBasedCheckpoint(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if (time.time() - self.last_checkpoint_time) >= self.interval_seconds:
            ...
            self.last_checkpoint_time = time.time()
            print(f"⏰ Time-based checkpoint saved: {{checkpoint_path}}")
...
echo "🚀 Starting Verda Instance Setup..."
echo "📅 $(date)"
