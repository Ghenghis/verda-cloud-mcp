...
class MCPTestSuite:
    ...
    async def test_check_availability(self) -> tuple:
        ...
        try:
            ...
            return True, f"A6000 available: {result.available} at {result.location or 'N/A'}"
        except Exception as e:
            return False, str(e)
    ...
    async def test_ssh_module(self) -> tuple:
        ...
        try:
            from .ssh_tools import PARAMIKO_AVAILABLE, SSHManager
    ...
    async def test_remote_file_ops(self, instance_ip: str) -> tuple:
        ...
        try:
            from .ssh_tools import ssh_write_file, ssh_read_file
    ...
    async def run_all_tests(self, instance_ip: Optional[str] = None) -> str:
        ...
        self.end_time = datetime.now()
        title = f"All Tests" + (f" ({instance_ip})" if instance_ip else " (No Instance)")
