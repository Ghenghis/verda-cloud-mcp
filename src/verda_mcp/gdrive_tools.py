...
class AutomatedDeployment:
    ...
    async def setup_training_environment(
        ...
        stdout, stderr, code = await loop.run_in_executor(
            ...
            lambda: manager.run_command(
                instance_ip,
                f"cd {workspace_dir} && unzip -o *.zip 2>/dev/null || echo 'No zip files or already extracted'",
            ),
        )
        ...
        for req in req_paths:
            stdout, stderr, code = await loop.run_in_executor(
                None,
                lambda: manager.run_command(instance_ip, f"test -f {req} && pip install -r {req} -q"),
            )
            if code == 0:
        ...
        stdout, stderr, code = await loop.run_in_executor(
            None,
            lambda: manager.run_command(instance_ip, "nvidia-smi --query-gpu=name,memory.total --format=csv"),
        )
        results.append(f"```\n{stdout}\n```")
    ...
    async def start_training(
        ...
        stdout, stderr, code = await loop.run_in_executor(None, lambda: manager.run_command(instance_ip, cmd))
...
async def gdrive_download_to_local(url_or_id: str, output_path: Optional[str] = None) -> str:
    ...
    try:
        path = await loop.run_in_executor(None, lambda: manager.download_file(url_or_id, output_path))
...
async def gdrive_download_folder_to_local(url_or_id: str, output_dir: Optional[str] = None) -> str:
    ...
    try:
        path = await loop.run_in_executor(None, lambda: manager.download_folder(url_or_id, output_dir))
...
async def transfer_local_to_verda(local_path: str, instance_ip: str, remote_path: str) -> str:
    ...
    try:
        await loop.run_in_executor(None, lambda: manager.upload_file(local_path, instance_ip, remote_path))
...
async def transfer_verda_to_local(instance_ip: str, remote_path: str, local_path: str) -> str:
    ...
    try:
        await loop.run_in_executor(None, lambda: manager.download_file(instance_ip, remote_path, local_path))
