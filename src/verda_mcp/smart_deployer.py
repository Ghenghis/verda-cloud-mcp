...
GPU_OPTIONS = {
    "GB300": {"vram": 288, "spot": 1.36, "on_demand": 5.45, "configs": [1, 2, 4], "power": 1800},
    "B300": {"vram": 262, "spot": 1.24, "on_demand": 4.95, "configs": [1, 2, 4, 8], "power": 1500},
    "B200": {"vram": 180, "spot": 0.95, "on_demand": 3.79, "configs": [1, 2, 4, 8], "power": 1200},
    "H200": {"vram": 141, "spot": 0.75, "on_demand": 2.99, "configs": [1, 2, 4, 8], "power": 990},
    "H100": {"vram": 80, "spot": 0.57, "on_demand": 2.29, "configs": [1, 2, 4, 8], "power": 990},
    "A100_80G": {"vram": 80, "spot": 0.32, "on_demand": 1.29, "configs": [1, 2, 4, 8], "power": 312},
    "A100_40G": {"vram": 40, "spot": 0.18, "on_demand": 0.72, "configs": [1, 8], "power": 312},
    "V100": {"vram": 16, "spot": 0.035, "on_demand": 0.14, "configs": [1, 2, 4, 8], "power": 125},
    "RTX_PRO_6000": {"vram": 96, "spot": 0.35, "on_demand": 1.39, "configs": [1, 2, 4, 8], "power": 91},
    "L40S": {"vram": 48, "spot": 0.23, "on_demand": 0.91, "configs": [1, 2, 4, 8], "power": 91},
    "RTX_6000_ADA": {"vram": 48, "spot": 0.21, "on_demand": 0.83, "configs": [1, 2, 4, 8], "power": 91},
    "A6000": {"vram": 48, "spot": 0.12, "on_demand": 0.49, "configs": [1, 2, 4, 8], "power": 38},
}
...
class SmartDeployer:
        ...
        for gpu_type, gpu_info in GPU_OPTIONS.items():
            ...
            for gpu_count in gpu_info.get("configs", [1]):
                for location in LOCATIONS:
                    try:
                        # Check spot
                        spot_avail = await self._check_availability(
                            client, gpu_type, gpu_count, location, is_spot=True
                        )
                        # Check on-demand
                        od_avail = await self._check_availability(
                            client, gpu_type, gpu_count, location, is_spot=False
                        )
    ...
    async def deploy_with_failsafes(
        ...
        for attempt_num, config in enumerate(configs_to_try):
            logger.info(f"🚀 Attempt {attempt_num + 1}: Deploying {config.gpu_count}x {config.gpu_type} ({config.mode.value}) at {config.location}...")
                ...
                try:
                    ...
                    if deploy_result.success:
                        ...
                        if verified:
                            ...
                            logger.info(f"✅ Deployment successful: {config.gpu_count}x {config.gpu_type} at {config.location}")
                            return result
                        else:
                            logger.warning(f"⚠️ Verification failed, retrying...")
            ...
            while (datetime.now() - start).total_seconds() < timeout:
                ...
                if status.status == "running":
                    ...
                    if expected_config.gpu_type.lower() in instance_type.lower():
                        ...
                        logger.warning(f"Instance type mismatch: expected {expected_config.gpu_type}, got {instance_type}")
                        return False
    ...
    async def _start_health_monitor(
        ...
        task = asyncio.create_task(
            self._health_monitor_loop(instance_id, instance_ip, config, checkpoint_minutes)
        )
        ...
        try:
            ...
            if instance_ip:
                try:
                    ...
                    ssh = get_ssh_manager()
                    stdout, _, code = ssh.run_command(instance_ip, "nvidia-smi --query-gpu=name --format=csv,noheader", timeout=30)
    ...
    def format_deployment_result(self, result: DeploymentResult) -> str:
        ...
        if result.success:
            ...
            if result.used_backup:
                lines.extend([
                    "",
                    f"⚠️ **Note**: Used backup option (primary was unavailable)",
                ])
